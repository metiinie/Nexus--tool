          ...
          habits.map((habit, index) => {
            ...
            return (
              <GlassCard key={habit.id} className="p-0 flex flex-col lg:flex-row overflow-hidden group border-white/5 hover:border-white/10 transition-all" hoverEffect>
                {/* Info Panel */}
                <div className="p-6 flex-1 flex items-center justify-between border-b lg:border-b-0 lg:border-r border-white/5">
                  <div className="flex items-center gap-5">
                    <div className={`p-4 rounded-2xl bg-${colorSet.id}-500/10 ${colorSet.text} group-hover:scale-110 transition-transform shadow-[0_0_20px_rgba(0,0,0,0.4)]`}>
                      <Activity size={28} />
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        <div className="flex items-center gap-1.5 bg-orange-500/10 px-2 py-0.5 rounded-full border border-orange-500/20">
                          ...
                          <span className="text-[10px] text-orange-400 font-black font-mono uppercase italic">{habit.streak} Day Continuous</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {/* Grid Panel */}
                <div className="p-5 flex items-center justify-between lg:justify-end gap-3 bg-white/[0.02]">
                  {last7Days.map((day) => {
                    ...
                    return (
                      <div key={day.dateStr} className="flex flex-col items-center gap-2.5">
                        ...
                        <button
                          ...
                          className={`
                              w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-500 relative group/tile
                              ${isCompleted
